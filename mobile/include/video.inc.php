...
if($itemid) {
	...
	($item && $item['status'] > 2 && $item['mobile'] == 1) or mobile_msg($L['msg_not_exist']);
	extract($item);
	$CAT = get_cat($catid);
	...
	$back_link = 'javascript:Dback(\''.mobileurl($moduleid, $catid).'\', \''.$DT_REF.'\', \'share|comment|club\');';
	$foot = '';
} else {
	if($kw) {
	...
	$condition = "status=3 AND mobile=1";
