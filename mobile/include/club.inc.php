...
if($itemid) {
	...
	if($gid) {
		...
		$condition = "status=3";
		if($keyword) $condition .= " AND title LIKE '%$keyword%'";
		if($keyword) $condition .= " OR d.content LIKE '%$keyword%'";
		if($catid) $condition .= $CAT ? " AND catid IN (".$CAT['arrchildid'].")" : " AND catid=$catid";
		if($areaid) $condition .= $ARE['child'] ? " AND areaid IN (".$ARE['arrchildid'].")" : " AND areaid=$areaid";
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} AS c LEFT JOIN {$table}_data AS d ON c.itemid=d.itemid WHERE $condition", 'CACHE');
		$items = $r['num'];
		$pages = mobile_pages($items, $page, $pagesize);
		$lists = array();
		if($items) {
			$order = $MOD['order'];
			$time = strpos($MOD['order'], 'add') !== false ? 'addtime' : 'edittime';
			$result = $db->query("SELECT * FROM {$table} AS c LEFT JOIN {$table}_data AS d ON c.itemid=d.itemid WHERE $condition ORDER BY $order LIMIT $offset,$pagesize");
			while($r = $db->fetch_array($result)) {
				if($kw) $r['title'] = str_replace($kw, '<b class="f_red">'.$kw.'</b>', $r['title']);
				$r['linkurl'] = 'index.php?moduleid=18&itemid='.$r['itemid'];
				$lists[] = $r;
			}
			$db->free_result($result);
		}
		...
		include template('club_search', 'mobile');
