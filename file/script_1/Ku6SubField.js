...
(function ($) {
	$.fn.KSubfield = function (options) {
		...
		function ElementControl (_thisObj, parentPage) {
			...
			if (_thisObj.divA >= 356) { // TOP
				setElementWH(parentPage.axesObjA, '356');
			} else {
				if (_thisObj.divA <= 130) {
					setElementWH(parentPage.axesObjA, '130');
				} else {
			...
			if (_thisObj.divB >= 356) { // BOTTOM
				setElementWH(parentPage.axesObjB, '356');
			} else {
				if (_thisObj.divB <= 130) {
					setElementWH(parentPage.axesObjB, '130');
				} else {
