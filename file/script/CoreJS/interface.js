...
* @param token 验证token
*/
function ObjvideoControl(){_alert('认证失败！',3);
	this.con_ip="vts";
	...
	showGuest:function(){
            ...
            if(!objC){
                ...
                if($("#myVideoBox").length<=0){
                    ...
                    var s=Dom.$C("script");
                    s.text="swfobject.embedSWF(\"/file/flash/5ShowShowLivePlayer.swf?roomId="+_show.roomId+"&rtmpHost="+_show.chat+"&cdn="+_show.cdnl+"&keyframe="+_show.zjg+"&height="+_show.height+"&width="+_show.width+"&fps="+_show.fps+"&bandwidth="+_show.zddk+"&quality="+_show.pz+"&rtmpPort=1935&appName=livepkgr\", \"JoyShowLivePlayer\", 428,316,\"10.0\", \"\", {},{wmode:\"transparent\",allowscriptaccess:\"always\"});guestflashSwf();JsInterface.guestdrag(\"myVideoBox\",\"dragguest\");";
				 ...
				 return false;
				  ...
				  switch(msgtype){
					  ...
					  case "5":
						...
						break;
					  case "6":
					  	this.getChatOnline(msgObject);//1
					    break;
					  case "7": //在线用户信息更新
	    ...
	    if(naction==0){ //上线
			try{
			  if(data){
				...
				//$.ajax({url:"/index.php/Show/getCard",success:function(data){var data=eval("("+data+")");alert(data['data']['daoju4'])}});
				
				//$("#chat_hall").append("<span>"+user['userName']+"</span>");
				...
				if(user["userid"]>0){ //不是游客
					if(user["h"]==0 && user["userid"]!=_show.emceeId){ //1: 隐身 0：显示
						...
						if(user["userType"]==10){ //巡管
							...
							str="<p class=\"tx_focus\">欢迎 <span class='cracy cra"+user["richlevel"]+"'></span><a class=\"chatuser\" id="+user["userid"]+" gn="+user["goodnum"]+" href=\"javascript:void(0);\">"+decodeURIComponent(user["username"])+"</a> <img src=\"/member/image/cz01.gif\"/></p>";
						}else if(user["richlevel"]>18 && user["richlevel"]<=21){
							str="<p class=\"tx_focus\">欢迎 <span class='cracy cra"+user["richlevel"]+"'></span><a class=\"chatuser\" id="+user["userid"]+" gn="+user["goodnum"]+" href=\"javascript:void(0);\">"+decodeURIComponent(user["username"])+"</a> <img src=\"/member/image/cz02.gif\"/></p>";
						}else if(user["richlevel"]>21 && user["richlevel"]<=26){
							str="<p class=\"tx_focus\">欢迎 <span class='cracy cra"+user["richlevel"]+"'></span><a class=\"chatuser\" id="+user["userid"]+" gn="+user["goodnum"]+" href=\"javascript:void(0);\">"+decodeURIComponent(user["username"])+"</a> <img src=\"/member/image/cz03.gif\"/></p>";
						}else if(user["richlevel"]>26 && user["richlevel"]<=30){
							str="<p class=\"tx_focus\">欢迎 <span class='cracy cra"+user["richlevel"]+"'></span><a class=\"chatuser\" id="+user["userid"]+" gn="+user["goodnum"]+" href=\"javascript:void(0);\">"+decodeURIComponent(user["username"])+"</a> <img src=\"/member/image/cz04.gif\"/></p>";
						}else if(user["richlevel"]>30){
							str="<p class=\"tx_focus\">欢迎 <span class='cracy cra"+user["richlevel"]+"'></span><a class=\"chatuser\" id="+user["userid"]+" gn="+user["goodnum"]+" href=\"javascript:void(0);\">"+decodeURIComponent(user["username"])+"</a> <img src=\"/member/image/cz05.gif\"/></p>";
						}else{
						   return;
						}
					}
				 	...
				 	// 游客进入
					str="<p class=\"tx_focus\">系统消息:"+decodeURIComponent(data['uname'])+" 进入房间</p>";	
				 }
			  }
			  return str;
			}catch(e){}	
	...
	showSendMsg:function(data){ //msgType:2
		...
		try{
				if(data){
					...
					if(data["action"]==0){ //公聊
						var toAllSay = data["equipment"]==undefined?'<p><span>'+time+'</span>'+icon+'<a class=\"chatuser\" gn="'+ugood+'" id='+uid+' href="javascript:void(0);">'+uname+'</a>'+cugood+': '+tempMsg+'</p>':'<p><span>'+time+'</span>'+icon+'<a class=\"chatuser\" gn="'+ugood+'" id='+uid+' href="javascript:void(0);"><img src="/member/image/mobile.png"/>'+uname+'</a>'+cugood+': '+tempMsg+'</p>';
		...
		try{
			...
			switch(Saction)
			{
				  case 33:  //送红包
				    ...
				    str='<p><span>'+data["timestamp"]+'</span><a href="javascript:void(0);" class=\"chatuser\" gn='+obj_box["userNo"]+' id='+obj_box["userId"]+'> '+decodeURIComponent(obj_box["userName"])+' </a>'+this.chatgnum(obj_box["userNo"])+' 送了红包一个<img src="/member/image/hb.gif" /></p>';
				   ...
				   case 4:  //抢座
				       ...
				       var Bitcoin=obj_box["seatPrice"]*100;
					   var seatstr='<p><span>'+ data["timestamp"] +'</span>' + obj_box["userNick"]+'花了'+Bitcoin+'个秀币抢'+obj_box["seatPrice"]+'沙发成功'+ ' <img src="/member/image/gift/sofa.png" /></p>';
					   ...
					   var gift_history='<li><span>'+'</span><img src="/member/image/gift/sofa.png" width="24" height="24"/><em>沙发</em><a title='+decodeURIComponent(obj_box["userNick"])+' href="/'+obj_box["userNo"]+'" target="_blank">'+ (gifthisNum+1)+ '. ' +decodeURIComponent(obj_box["userNick"])+'</a></li>';
					   $('#gift_history').append(gift_history);
					   $('#get_sofa').hide();
					   Chat.getUserBalance();//用户秀币更新	
					   Chat.getRankByShow();//本场排行
				  ...
				  case 12: //礼物、沙发、礼物周星、飞屏  大公告 
				  		...
				  		var url = "/index.php/Show/get_gift_list";
				  		$.ajax({
				  			url:url,
				  			success:function(data)
				  			{
...
if(html.indexOf('0个',1)==-1){
				...
				case 55: //特殊礼物发放
					...
					if(prizeid=="1"){
						prizestr="金牌";
						prizeurl="/member/image/active/lday/images/1.png"
						moneystr="50000";
					}else if(prizeid=="2"){
						prizestr="银牌";
						prizeurl="/member/image/active/lday/images/2.png"
						moneystr="30000";
					}else if(prizeid=="3"){
						prizestr="铜牌";
						prizeurl="/member/image/active/lday/images/3.png"
						moneystr="10000";
					}else if(prizeid=="4"){
						prizestr="奖杯";
						prizeurl="/member/image/active/lday/images/J1.png"
						moneystr="50000";
					}else if(prizeid=="5"){
						prizestr="奖杯";
						prizeurl="/member/image/active/lday/images/J2.png"
						moneystr="30000";
					}else if(prizeid=="6"){
						prizestr="奖杯";
						prizeurl="/member/image/active/lday/images/J3.png"
						moneystr="10000";
					}
					...
					break;
                                case 47: //贴条操作
                                    ...
                                    if(obj_box['add']==1){
                                        ...
                                        $("#tt_"+data["touid"]).html('<img src="/member/image/note/bandingImg/tt'+obj_box['stk']+'.png" />');
...
//                                    try{ console.log("魔法兔子个人房状态..."+_game.rabbitclosed);}catch(e){ }
	...
	endLive:function(){ //end play
		...
		for(i=1;i<=5;i++){
			 ...
			 sofa_o.attr({'seatnum':0,'src':'/member/image/default1.gif','title':''});
		 }
		 $('#get_sofa').hide();
		 $("#usersonglist").html('');
	...
	getChatOnline:function(data){ //fet onlinelist
        Dom.$getid("flashCallChat")._notification();
		//clear_data deal
		this.arrPeople=[];
		this.arrManage=[];
		this.cntManage=0;
		...
		for(var key in arrManage){//manage
			...
			if(1==1 || mitem["userType"]==50){//显身
				...
				if(mitem["sellm"]!=0){//代理标准
					strMin2+=" <img src=\"/member/image/sell.gif\" width=\"35\" height=\"16\"/>";	
				}
			...
			if(1==1 || pitem["userType"]==50){//显身
				...
				if(pitem["userType"]==5){ //游客僵尸
					...
					if(pitem["userType"]==50){ //主播
						...
						if(pitem["richlevel"]>0){
							//strOn1+=" <span class='cracy cra"+pitem["richlevel"]+"'></span>";
                                                        ...
                                                        strOn1+="<img class='cracy cra"+pitem["richlevel"]+"' src='/member/image/sx"+sx+".gif'>";//123456
						}
					}
					if(pitem["vip"]!=0){//VIP
					...
					if(pitem["sellm"]!=0){//代理标准
						strOn2+=" <img src=\"/member/image/sell.gif\" width=\"35\" height=\"16\"/>";	
					}
...
//                                        }
                                        ...
                                        if(ttstk==''){
                                            ...
                                            strOn0+='<span id="tt_'+pitem["userid"]+'" style="width:53px; height:32px;position:absolute; left:70px;"><img src="/member/image/note/bandingImg/tt'+pitem['stk']+'.png"></span>';
                                        }
...
//                            try{ console.log("1贴条数组长度..."+tieTiaoArray.length);}catch(e){ }
		...
		$('#lm2_2').find('cite').html(this.cntPeople);
		$("#content2_2").html(userArray.join(""));
//		$('#content2_2').append("<li style='text-align:right;width:122px;'><a>游客"+this.guePeople+"人</a></li>");
		...
		if(data){
			...
			if(userid<0){//visistor
				...
				if(this.initnum<this.minCount){
					...
					$('#content2_2').append("<li id='online_"+udata["userid"]+"' tid='"+udata["userid"]+"' order='"+udata["sortnum"]+"' utype='"+udata["userType"]+"' title='"+decodeURIComponent(udata["username"])+"'><p><a>"+decodeURIComponent(udata["username"])+"<img style='width:44px'  class='tou_xiang' src='/passport/avatar.php?uid="+udata["h"]+"&size=middle'/></a></p></li>");	
				...
				{ //people
					...
					this.chatPeople();
                                        for(var i in tieTiaoArray){
                                            $("#tt_"+tieTiaoArray[i][0]).html('<img src="/member/image/note/bandingImg/tt'+tieTiaoArray[i][2]+'.png">');
                                        }
				}
				if(utype==40){
					meq=this.getloc(this.arrManage,userid);
	...
	openSmallf:function(uid,uname,ugood){  //this.initflash
		...
		loadf=that.loadSwf('/member/image/smallflash.swf',50,50,that.inf);
		fJson.l=oPos.left;
	...
	openSmallg:function(gid,uname,ugood,gcount,gname,gimg){ //initgift
		var that=this,oPos=$('.chat_online').offset(),tpl="<div class=\"tit\"><div class=\"txt\"></div></div><div class=\"g_gif\"><img src=\"/member/image/smallgift.gif\" /></div><div class=\"sticon\"></div>",domg=Dom.$C("div"),gJson={};
		that.ing++;
...
function fullscreen(){
	Dom.$swfId("JoyShowLivePlayer").width = window.screen.availWidth;
	Dom.$swfId("JoyShowLivePlayer").height = window.screen.availHeight;
	$("#flash_full_btn").html('<a style="color:#fff" onclick="exitfullscreen()" href="javascript:void(0);">退出全屏</a>');
	$("#head").hide();
	$("#destoon_space").hide();
	$("#container").css('height',$(document).height());
	$("#container").css('background','none');
	$("#container .wrap").css('width','auto');
	$("#container .wrap").css('margin-top','0');
	$("#container .wrap2").css('margin-top','0');
	$("#container .wrap .main-flash").css('margin-left',"0px");
	$(".main_chat").hide();
}
function exitfullscreen(){
	Dom.$swfId("JoyShowLivePlayer").width = "480";
	Dom.$swfId("JoyShowLivePlayer").height = "360";
	$("#flash_full_btn").html('<a style="color:#fff" onclick="fullscreen()" href="javascript:void(0);">全屏</a>');
	$("#head").show();
	$("#destoon_space").show();
	$("#container").css('height',"680px");
	$("#container").css('background','url(/member/image/showbackground.jpg) top center no-repeat #000');
	$("#container .wrap").css('width','1200px');
	$("#container .wrap2").css('margin-top','12px');
	$("#container .wrap .main-flash").css('margin-left',"10px");
	$(".main_chat").show();
}
/**
 * 初始化User Cache
...
var InitCache=function(){
	_vc = "";
	// var url = "/index.php/Show/show_showData/eid/"+_show.emceeId+"/rid/"+_show.roomId+Sys.ispro+"/t/"+Math.random();
	var url = "/member/liveonline.php?action=showdata&rid="+_show.roomId+"&t="+Math.random();
	$.getJSON(url,function(json){
		if(json && json.code==0){
			function defineVars(json){
				var udata=json.data;
				var user=udata.userInfo,node=udata.nodeInfo,show=udata.showInfo;
				_show.up=node.up;_show.down=node.down;_show.fmsPort = node.fmsPort;				
				_show.chat=node.chat==""?"csp":node.chat;_show.userId=user.userId,_show.emceeId=user.emceeId;
				_show.version=udata.version;_show.admin=user.admin;_show.deny=show.deny;
				_show.showId=show.showId;_show.is_public=show.isPublicChat;_show.closed=show.closed;
				_show.local=0;_show.showTime=show.showTime;			
				// if(Sys.ispro!=""){
			...
			function loadFlash(){
				...
				if(_show.emceeId==_show.userId){
					_script.text="swfobject.embedSWF(\"/file/flash/5ShowCamLivePlayer.swf?roomId="+_show.roomId+"&rtmpHost="+_show.chat+"&cdn="+_show.cdn+"&keyframe="+_show.zjg+"&fps="+_show.fps+"&bandwidth="+_show.zddk+"&width="+_show.width+"&height="+_show.height+"&quality="+_show.pz+"&rtmpPort=1935&appName=livepkgr\",\"JoyCamLivePlayer\",480,360,\"10.0\", \"\",{},{quality:\"high\",wmode:\"opaque\",allowscriptaccess:\"always\"});"   
				}else{
					_script.text="var showflash = swfobject.embedSWF(\"/file/flash/5ShowShowLivePlayer.swf?roomId="+_show.roomId+"&rtmpHost="+_show.chat+"&cdn="+_show.cdnl+"&keyframe="+_show.zjg+"&fps="+_show.fps+"&bandwidth="+_show.zddk+"&width="+_show.width+"&height="+_show.height+"&quality="+_show.pz+"&rtmpPort=1935&appName=livepkgr\",\"JoyShowLivePlayer\", 480,360,\"10.0\", \"\",{},{quality:\"high\",wmode:\"opaque\",allowscriptaccess:\"always\"});"     

					var control_btn = Dom.$C('div');
					control_btn.id = 'flash_full_btn';
					control_btn.style.position = 'absolute';
					control_btn.style.top = '10px';
					control_btn.style.right = '10px';
					control_btn.innerHTML = '<a style="color:#fff" onclick="fullscreen()" href="javascript:void(0);">全屏</a>';
					$('#livebox').append(control_btn);
					Dom.$getid('livebox').onmouseover = function(){
						// alert("sdf");
					}
				}
			...
			function setRoom(){
				...
				for(var i=1;i<=4;i++){
					...
					if(i<5){
					   strsofa+='<li style="float:left;margin-top:0px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t'+i+'"><img src="'+sf.icon+'" title="'+sf.nick+'" seatnum="'+sf.count+'"/><span seatid="'+i+'">抢座</span> </a></li>';
						  
						
					 }else if(i==4){
						 strsofa+='<li style="float:left;margin-left:320px;margin-top:'+(7-i)*7+'px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t'+i+'"><img src="'+sf.icon+'" title="'+sf.nick+'" seatnum="'+sf.count+'"/><span seatid="'+i+'">抢座</span> </a></li>';
					  }else{
						 strsofa+='<li style="float:left;margin-top:'+(7-i)*7+'px;"><a href="javascript:void(0)" data-seat="0" class="seatlight t'+i+'"><img src="'+sf.icon+'" title="'+sf.nick+'" seatnum="'+sf.count+'"/><span seatid="'+i+'">抢座</span> </a></li>';
					  }
...
$('#tietiaob').on("mouseenter",function(e){
        $(this).addClass("hover");
        clearTimeout(ccc);
