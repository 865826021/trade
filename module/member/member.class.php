...
class member {
	...
	var $table_member_live;
	var $table_member_liveset;
	var $table_company;
    ...
    function member() {
		...
		$this->table_member_live = $db->pre.'member_live';
		$this->table_member_liveset = $db->pre.'member_liveset';
	...
	function is_member($member) {
		...
		if($this->userid) {
			...
			if($member['groupid'] == 6 && !$this->is_company($member['company'])) return false;
	...
	function add($member) {
		...
		/** 修改添加直播间 by zhang start **/
		if ($member['islive'] == 1) {
			$this->db->query("INSERT INTO {$this->table_member_liveset}(userid) VALUES('$this->userid')");
		}
		/** 修改添加直播间 by zhang end **/ 
		return $this->userid;
	}
	...
	function edit($member)	{
		...
		$member = $this->set_member($member);
		$r = $this->get_one();
		$member_fields = array('company','passport','sound','email','msn','qq','ali','skype','gender','truename','mobile','department','career','groupid','areaid', 'edittime','black','bank','banktype','branch','account','vemail','vmobile','vbank','vtruename','vcompany','vtrade','trade','support','inviter','islive');
		...
		$member['userid'] = $this->userid;
		$member['vip'] = $r['vip'];
		/** 修改直播间 by zhang start **/
		if ($member['islive'] == 1) {
			$this->db->query("INSERT INTO {$this->table_member_liveset} (userid) VALUES('$this->userid')");
		}else{
			$this->db->query("delete from {$this->table_member_liveset} where userid=$this->userid");
		}
		/** 修改直播间 by zhang end **/ 
	...
	function get_paipian($condition){
		global $pages, $page, $pagesize, $offset, $pagesize, $CFG, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->table_member_live} WHERE $condition");
			$items = $r['num'];
		}
		$pages = pages($items, $page, $pagesize);
		$members = array();
		$result = $this->db->query("SELECT * FROM {$this->table_member_live} ml left join {$this->table_member} m on ml.userid=m.userid WHERE $condition LIMIT $offset,$pagesize");
		while($r = $this->db->fetch_array($result)) {
			$r['logindate'] = timetodate($r['logintime'], 5);
			$r['regdate'] = timetodate($r['regtime'], 5);
			$members[] = $r;
