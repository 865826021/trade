...
class mall {
	...
	function pass($post, $CAT) {
		...
		if ($CAT ['has_price']) {
			if (dround ( $post ['step'] ['p1'] ) < 0.1)
				return $this->_ ( lang ( 'message->pass_mall_price' ) );
		}
		if ($CAT ['has_amount']) {
			if (intval ( $post ['amount'] ) < 1)
				return $this->_ ( lang ( 'message->pass_mall_amount' ) );
		}
		if (! $post ['thumb'])
			return $this->_ ( lang ( 'message->pass_thumb' ) );
		if (! $post ['content'])
			return $this->_ ( lang ( 'message->pass_content' ) );
	...
	function set($post) {
		...
		is_url($post['thumb3']) or $post['thumb3'] = '';
		is_url($post['thumb4']) or $post['thumb4'] = '';
		is_url($post['thumb5']) or $post['thumb5'] = '';
		is_url($post['thumb6']) or $post['thumb6'] = '';
		is_url($post['thumb7']) or $post['thumb7'] = '';
		is_url($post['thumb8']) or $post['thumb8'] = '';
		is_url($post['thumb9']) or $post['thumb9'] = '';
		...
		if ($this->itemid) {
			...
			if ($post ['thumb3'])
				$new .= '<img src="' . $post ['thumb3'] . '"/>';
			if ($post ['thumb4'])
				$new .= '<img src="' . $post ['thumb4'] . '"/>';
			if ($post ['thumb5'])
				$new .= '<img src="' . $post ['thumb5'] . '"/>';
			if ($post ['thumb6'])
				$new .= '<img src="' . $post ['thumb6'] . '"/>';
			if ($post ['thumb7'])
				$new .= '<img src="' . $post ['thumb7'] . '"/>';
			if ($post ['thumb8'])
				$new .= '<img src="' . $post ['thumb8'] . '"/>';
			if ($post ['thumb9'])
				$new .= '<img src="' . $post ['thumb9'] . '"/>';
			$r = $this->get_one ();
			$old = $r ['content'];
			if ($r ['thumb'])
				$old .= '<img src="' . $r ['thumb'] . '"/>';
			...
			if ($r ['thumb3'])
				$old .= '<img src="' . $r ['thumb3'] . '"/>';
			if ($r ['thumb4'])
				$old .= '<img src="' . $r ['thumb4'] . '"/>';
			if ($r ['thumb5'])
				$old .= '<img src="' . $r ['thumb5'] . '"/>';
			if ($r ['thumb6'])
				$old .= '<img src="' . $r ['thumb6'] . '"/>';
			if ($r ['thumb7'])
				$old .= '<img src="' . $r ['thumb7'] . '"/>';
			if ($r ['thumb8'])
				$old .= '<img src="' . $r ['thumb8'] . '"/>';
			if ($r ['thumb9'])
				$old .= '<img src="' . $r ['thumb9'] . '"/>';
			delete_diff ( $new, $old );
		} else {
			$post ['ip'] = $DT_IP;
		}
	...
	function add($post) {
		...
		if ($post ['status'] == 3 && $post ['username'] && $MOD ['credit_add']) {
			...
			credit_record ( $post ['username'], $MOD ['credit_add'], 'system', lang ( 'my->credit_record_add', array (
					$MOD ['name'] 
			) ), 'ID:' . $this->itemid );
		}
		clear_upload ( $post ['content'] . $post ['thumb'] . $post ['thumb1'] . $post ['thumb2'] . $post ['thumb3'] . $post ['thumb4'] . $post ['thumb5'] . $post ['thumb6'] . $post ['thumb7'] . $post ['thumb8'] . $post ['thumb9'], $this->itemid );
		return $this->itemid;
	}
	function edit($post) {
	...
	function tohtml($itemid = 0, $catid = 0) {
		...
		if ($MOD ['show_html'] && $itemid)
			tohtml ( 'show', $module, "itemid=$itemid" );
	}
	...
	function delete($itemid, $all = true) {
		...
		if (is_array ( $itemid )) {
			...
			if ($all) {
				...
				if ($r ['thumb3'])
					delete_upload ( $r ['thumb3'], $userid );
				if ($r ['thumb4'])
					delete_upload ( $r ['thumb4'], $userid );
				if ($r ['thumb5'])
					delete_upload ( $r ['thumb5'], $userid );
				if ($r ['thumb6'])
					delete_upload ( $r ['thumb6'], $userid );
				if ($r ['thumb7'])
					delete_upload ( $r ['thumb7'], $userid );
				if ($r ['thumb8'])
					delete_upload ( $r ['thumb8'], $userid );
				if ($r ['thumb9'])
					delete_upload ( $r ['thumb9'], $userid );
				if ($r ['content'])
				...
				if ($r ['username'] && $MOD ['credit_del']) {
					...
					credit_record ( $r ['username'], - $MOD ['credit_del'], 'system', lang ( 'my->credit_record_del', array (
							$MOD ['name'] 
					) ), 'ID:' . $this->itemid );
				}
			}
	...
	function check($itemid) {
		...
		if (is_array ( $itemid )) {
			...
			if ($MOD ['credit_add'] && $item ['username'] && $item ['hits'] < 1) {
				...
				credit_record ( $item ['username'], $MOD ['credit_add'], 'system', lang ( 'my->credit_record_add', array (
						$MOD ['name'] 
				) ), 'ID:' . $this->itemid );
			}
