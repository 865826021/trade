...
if (in_array ( $action, array (
	...
	$CAT = get_cat ( $catid );
	if ($CAT ['parentid']) {
		$find_set = $catid . ',' . str_replace ( '0,', '', $CAT ['arrparentid'] );
	} else {
		$find_set = $catid;
	}
	$result = $db->query ( "SELECT * FROM {$DT_PRE}fields WHERE catid IN (" . $find_set . ") ORDER BY listorder ASC LIMIT 20" );
...
switch ($action) {
	case 'category' :
		...
		$EXP = array ();
		if ($_username && in_array ( $action, array (
				'add',
				'edit' 
		) )) {
		...
		$head_title = lang ( $L ['module_manage'], array (
				$MOD ['name'] 
		) );
		include template ( 'my_category', 'member' );
		exit ();
		break;
	case 'add' :
		if ($MG ['mall_limit'] && $limit_used >= $MG ['mall_limit'])
			dalert ( lang ( $L ['info_limit'], array (
					$MG [$MOD ['module'] . '_limit'],
					$limit_used 
			) ), $_userid ? $MODULE [2] ['linkurl'] . $DT ['file_my'] . '?mid=' . $mid : $MODULE [2] ['linkurl'] . $DT ['file_my'] );
		if ($MG ['day_limit']) {
			$today = $today_endtime - 86400;
			$r = $db->get_one ( "SELECT COUNT(*) AS num FROM {$table} WHERE $sql AND addtime>$today" );
			if ($r && $r ['num'] >= $MG ['day_limit'])
				dalert ( lang ( $L ['day_limit'], array (
						$MG ['day_limit'] 
				) ), $_userid ? $MODULE [2] ['linkurl'] . $DT ['file_my'] . '?mid=' . $mid : $MODULE [2] ['linkurl'] . $DT ['file_my'] );
		}
		...
		if ($FD)
			fields_html ( '<td class="tl">', '<td class="tr">', $item );
		if ($submit) {
			...
			if ($do->pass ( $post, $CAT )) {
				...
				if ($could_elite && isset ( $elite ) && $post ['thumb'] && $_credit > $MOD ['credit_elite']) {
					...
					credit_record ( $_username, - $MOD ['credit_elite'], 'system', lang ( $L ['credit_record_elite'], array (
							$MOD ['name'] 
					) ), $post ['title'] );
				}
				...
				if ($fee_add) {
					if ($fee_currency == 'money') {
						...
						money_record ( $_username, - $fee_add, $L ['in_site'], 'system', lang ( $L ['credit_record_add'], array (
								$MOD ['name'] 
						) ), 'ID:' . $do->itemid );
					} else {
						credit_add ( $_username, - $fee_add );
						credit_record ( $_username, - $fee_add, 'system', lang ( $L ['credit_record_add'], array (
								$MOD ['name'] 
						) ), 'ID:' . $do->itemid );
					}
				}
...
// 				$thumb = $thumb1 = $thumb2 = $thumb3 = $thumb4 = $thumb5 = $thumb6 = $thumb7 = $thumb8 = $thumb9 = '';
			} else {
	...
	case 'edit' :
		...
		if ($MG ['edit_limit'] && $DT_TIME - $item ['addtime'] > $MG ['edit_limit'] * 86400)
			message ( lang ( $L ['edit_limit'], array (
					$MG ['edit_limit'] 
			) ) );
		if ($submit) {
			if ($do->pass ( $post, $CAT )) {
				if (! $CAT || ! check_group ( $_groupid, $CAT ['group_add'] ))
					dalert ( lang ( $L ['group_add'], array (
							$CAT ['catname'] 
					) ) );
	...
	case 'refresh' :
		...
		if ($MOD ['credit_refresh'] && $s) {
			...
			credit_record ( $_username, - $credit, 'system', lang ( $L ['credit_record_refresh'], array (
					$MOD ['name'] 
			) ), lang ( $L ['refresh_total'], array (
					$s 
			) ) );
		}
		$msg = lang ( $L ['refresh_success'], array (
				$s 
		) );
		if ($f)
			$msg = $msg . ' ' . lang ( $L ['refresh_fail'], array (
					$f 
			) );
		dmsg ( $msg, $forward );
		break;
	...
	default :
		...
		if ($minorders)
			$condition .= " AND orders>=$minorders";
		if ($maxorders)
			$condition .= " AND orders<=$maxorders";
		if ($minsales)
			$condition .= " AND sales>=$minsales";
		if ($maxsales)
