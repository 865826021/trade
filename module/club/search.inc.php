...
if($DT_QST) {
	...
	if($action == 'group') {
		...
		echo "SELECT * FROM {$table}_group WHERE {$condition} ORDER BY itemid DESC LIMIT {$offset},{$pagesize}";
		...
		$tags = $do->get_list($condition, $MOD['order'], $DT['cache_search'] ? 'CACHE' : '', true);
		if($tags && $kw) {
			foreach($tags as $k=>$v) {
