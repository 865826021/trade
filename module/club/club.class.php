...
class club {
    ...
    function club($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','areaid','gid','video','level','title','style','fee','introduce','thumb','tag','status','hits','username','passport','addtime','editor','edittime','ip','template', 'linkurl','filepath','note');
    }
	...
	function get_list($condition = 'status=3', $order = 'addtime DESC', $cache = '', $search = false) {
		global $MOD, $pages, $page, $pagesize, $offset, $items, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			$r = $this->db->get_one("SELECT COUNT(*) AS num FROM {$this->table} AS c LEFT JOIN {$this->table_data} AS d ON c.itemid=d.itemid WHERE $condition", $cache);
			$items = $r['num'];
		}
		...
		if ($search){
			$result = $this->db->query("SELECT c.*,d.content FROM {$this->table} AS c LEFT JOIN {$this->table_data} AS d ON c.itemid=d.itemid WHERE $condition ORDER BY $order LIMIT $offset,$pagesize", $cache);
		} else {
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				$result = $this->db->query("SELECT * FROM {$this->table}_reply WHERE tid=$itemid");
				while($rr = $this->db->fetch_array($result)) {
					if($rr['content']) delete_local($rr['content'], get_user($rr['username']));
				}
				$this->db->query("DELETE FROM {$this->table}_reply WHERE tid=$itemid");
			}
		}
	}
