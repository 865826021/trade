...
$do->tb = $tb;
$menus = array (
    array('添加字段', '?file='.$file.'&tb='.$tb.'&catid='.$catid.'&action=add'),
    array('字段列表', '?&file='.$file.'&tb='.$tb.'&catid='.$catid),
);
$this_forward = '?moduleid='.$moduleid.'&file='.$file.'&tb='.$tb.'&catid='.$catid;
switch($action) {
	case 'add':
		if($submit) {
			if($do->pass($post)) {
	...
	break;
	default:
		$lists = $do->get_list("tb='$tb' AND catid='$catid'");
		cache_fields($tb);
		include tpl('fields');
	break;
}
