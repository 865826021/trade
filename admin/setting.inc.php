...
switch($action) {
	...
	case 'mail':
		...
		if($mail_type == 'sc') {
			...
			$body = '<b>恭喜！您的站点['.$DT['sitename'].']邮件发送设置成功！</b>';
		}	
	...
	default:
		...
		if($submit) {
			...
			$setting['admin_week'] = isset($setting['admin_week'])?implode(',', $setting['admin_week']):'';
			$setting['check_week'] = isset($setting['check_week'])?implode(',', $setting['check_week']):'';
