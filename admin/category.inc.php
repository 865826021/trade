		...
		if($submit) {
			...
			if(strpos($category['catname'], "\n") === false) {
				...
				foreach($catnames as $catname) {
					...
					if(!$catname) continue;
					$catname_array = explode('|', $catname);
					if (count($catname_array) > 1){
						list($letter, $catname_cn, $catname_en) = explode('|', $catname);
					} else {
						$catname_cn = $catname;
						$catname_en = '';
						$letter = '';
					}
					$category['catname'] = $catname_cn;
					$category['catname_en'] = $catname_en;
					$category['catdir'] = '';
...
class category {
	...
	function add($category)	{
		...
		$category['letter'] = preg_match("/^[a-z]{1}+$/i", $category['letter']) ? strtoupper($category['letter']) : '';
	...
	function update($category) {
		...
		foreach($category as $k=>$v) {
			...
			$v['letter'] = preg_match("/^[a-z0-9]{1}+$/i", $v['letter']) ? strtoupper($v['letter']) : '';
	...
	function get_letter($catname, $letter = true) {
		return $letter ? strtoupper(substr(gb2py($catname), 0, 1)) : str_replace(' ', '', gb2py($catname));
	}
